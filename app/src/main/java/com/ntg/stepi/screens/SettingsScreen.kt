...
import com.ntg.stepi.MainActivity.Companion.sensorType
...
private fun  Content(navHostController: NavHostController, paddingValues: PaddingValues, userDataViewModel: UserDataViewModel){
    ...
    language.value = userDataViewModel.getLanguage().collectAsState(initial = "").value
    ...
    LazyColumn(modifier = Modifier.padding(paddingValues), horizontalAlignment = Alignment.CenterHorizontally){
        ...
        item {
            ...
            Text(modifier = Modifier
                .fillMaxWidth()
                .padding(horizontal = 24.dp)
                .padding(top = 24.dp, bottom = 8.dp), text = stringResource(id = R.string.language), style = fontBold14(MaterialTheme.colors.primary))


            ItemOption(text = if (language.value == "fa") stringResource(id = R.string.persian) else if (language.value == "ar") stringResource(id = R.string.arabic) else stringResource(id = R.string.english), onClick = {
                navHostController.navigate(Screens.SelectLanguageScreen.name)
            })
